...
/**
 * Registers the {@code entity:rotation/changed/remove} MSMP method.
 *
 * <p>Removes entities from the rotation change notification tracker
 * and clears their cached last-rotation values.</p>
 */
public class RotationChangedRemove {

    private RotationChangedRemove() {}

    public static void register(MSMPNamespace namespace) {

        namespace.method("rotation/changed/remove")
            .description("Remove entities from the rotation change notification tracker")
            ...
            .register((server, client, params) -> {
                ...
                for (EntityRequest entry : params.entities()) {
                    try {
                        ...
                        RPCConnectionLogger.warning(client.connectionId(), "entity:rotation/changed/remove - " + e.getMessage());
                        throw e;
                    }
                }

                entityTracker.remove(uuids);
                uuids.forEach(NotificationRotationChanged.LAST_ROTATIONS::remove);

                RPCConnectionLogger.info(client.connectionId(),
                    "entity:rotation/changed/remove - removed %s".formatted(uuids));

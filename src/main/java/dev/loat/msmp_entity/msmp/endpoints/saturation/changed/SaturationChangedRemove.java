...
/**
 * Registers the {@code entity:saturation/changed/remove} MSMP method.
 *
 * <p>Removes entities from the saturation change notification tracker
 * and clears their cached last-recorded values.</p>
 */
public class SaturationChangedRemove {

    private SaturationChangedRemove() {}

    public static void register(MSMPNamespace namespace) {

        namespace.method("saturation/changed/remove")
            .description("Remove entities from the saturation change notification tracker")
            ...
            .register((server, client, params) -> {
                ...
                for (EntityRequest entry : params.entities()) {
                    try {
                        ...
                        RPCConnectionLogger.warning(client.connectionId(), "entity:saturation/changed/remove - " + e.getMessage());
                        throw e;
                    }
                }

                entityTracker.remove(uuids);
                uuids.forEach(NotificationSaturationChanged.LAST_SATURATION::remove);

                RPCConnectionLogger.info(client.connectionId(),
                    "entity:saturation/changed/remove - removed %s".formatted(uuids));
